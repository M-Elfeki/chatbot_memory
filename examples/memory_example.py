#!/usr/bin/env python3
"""
Minimal example for memory tool with GPT-OSS.

This example demonstrates how the LLM can automatically use the memory tool
to store and retrieve information based on semantic similarity.

Usage:
    python examples/memory_example.py "your prompt here" [--no-stream] [--temp TEMP] [--top-p TOP_P] [--reasoning-level LEVEL]

Example:
    python examples/memory_example.py "Remember that I prefer Python over Java"
    python examples/memory_example.py "What programming languages do I prefer?" --temp 0.8 --top-p 0.9
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Minimal memory tool example with GPT-OSS",
    ...
    parser.add_argument(
        "--clear-memory",
        action="store_true",
        help="Clear memory before running (for testing)"
    )
    
    args = parser.parse_args()
    
    # Clear memory if requested
    if args.clear_memory and os.path.exists(args.storage_path):
        shutil.rmtree(args.storage_path)
        print(f"Cleared memory store: {args.storage_path}")
    ...
    def on_tool_call(tool_name, arguments):
        ...
        print(f"\n[TOOL CALL] {tool_name}({json.dumps(arguments, indent=2)})\n", flush=True)
    
    # Custom tool executor that adds storage_path
    def tool_executor(tool_name, arguments):
        """Tool executor that adds storage_path to arguments."""
        if "storage_path" not in arguments:
            arguments["storage_path"] = args.storage_path
    ...
    print(f"Memory storage: {args.storage_path}")
    print("=" * 80)
    print("\n[RESPONSE]\n")
